...
# run either to save plot (for later use) or get the dataframe with clusters annotated
non_all <- mainPipeline(4, TRUE, FALSE, "all", "df", c("C", "D"))
clock_all <- mainPipeline(4, TRUE, TRUE, "all", "df", c("A", "B"))
male_all <- mainPipeline(4, TRUE, FALSE, "male", "plot", c("A", "B"))
female_all <- mainPipeline(4, TRUE, FALSE, "female", "plot", c("C", "D"))
...
p3 <- grid.grabExpr(draw(ht, column_title = "Cluster Enrichment by Clocks", column_title_gp = gpar(fontsize=14, fontface="bold", fontfamily="Helvetica")))

ht2 <- clusterEnrichment(non_all, 4, all_list)
p4 <- grid.grabExpr(draw(ht2, column_title = "Cluster Enrichment by Windows", column_title_gp = gpar(fontsize=14, fontface="bold", fontfamily="Helvetica")))

fig2c <- ggarrange(p3, p4, labels=c("E", "F"))
```
