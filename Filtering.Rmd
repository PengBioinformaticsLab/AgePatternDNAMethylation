...
all_plot <- plot_grid(arranged_all, arranged_male, arranged_female, ncol=3, labels="AUTO")

# can't seem to combine these???
# another problem: blue dots seem to disappear when combining all 3 columns... idk


# extra annotation needed on final figure
row_labels <- grobTree(
  textGrob("All",    x = unit(0.22,   "npc"), y = unit(0.5, "npc"),
           rot = 0, gp = gpar(fontsize=12, fontface="bold")),
  textGrob("Male",   x = unit(0.55,   "npc"), y = unit(0.5, "npc"),
           rot = 0, gp = gpar(fontsize=12, fontface="bold")),
  textGrob("Female", x = unit(0.88,   "npc"), y = unit(0.5, "npc"),
           rot = 0, gp = gpar(fontsize=12, fontface="bold"))
)

combined <- as_ggplot(arrangeGrob(row_labels, all_plot,
                        nrow   = 2,
                        heights = c(1, 40)))


# will need manual conversion to pdf
ggsave("new_figs/supp_fig3.png",
       plot=combined,
       width=8,
       height=30,
       dpi=300)
...
p1 <- grid.grabExpr({
  ...
  draw(UpSet(m2, set_order = rownames(m2)))
})
...
all_plot <- plot_grid(combined_plot, arrangedPlots, nrow=2, rel_widths=c(1, 1), rel_heights=c(1.7, 4))
print(all_plot)
ggsave("new_figs/fig3.pdf", device=cairo_pdf, plot=last_plot(), width=14, height=12, dpi=300)
```
...
male_upset_grob <- grid.grabExpr({
  ht_up <- UpSet(m_up, set_order = rownames(m_up), pt_size = unit(1.5, "mm"), lwd=1)
...
pdf("new_figs/supp_fig3.pdf", width = 6, height = 12)
...

{
  ...
  #png("figs/circosA.png", width = 8, height=6, units="in", res=300)
  #p1 <- grid.grabExpr({
    plot.new() 
    ...
    upViewport()  # exit viewport
    # Draw the legend list on the left of the circle; adjust x/just as needed
...
# combine into single using cowplot

combined_plot <- plot_grid(p1, p2, p3, ncol=1)
print(combined_plot)
ggsave("new_figs/circos_all.pdf", device=cairo_pdf, plot = combined_plot, dpi = 300)
```

```{r}
p1_grob <- grid.grabExpr({
  plot.new()
  circle_size <- unit(1, "snpc")
  pushViewport(viewport(x = 0, y = 0.5,
                        ...
                        just = c("left","center")))
  par(omi = gridOMI(), new = TRUE)
  circos.initializeWithIdeogram(paste0("chr",1:22),"hg38")
  circos.genomicDensity(M1,           col="blue",  track.height=0.1)
  circos.genomicDensity(filtered_plt, col="red",   track.height=0.1)
  circos.genomicDensity(all_aaCpG_plt,col="green", track.height=0.1)
  upViewport()
  draw(packLegend(
    Legend(at="Whole Genome CpGs", type="points", legend_gp=gpar(col="blue")),
    Legend(at="Trimmed CpGs",      type="points", legend_gp=gpar(col="red")),
    Legend(at="All aaCpGs",        type="points", legend_gp=gpar(col="green"))
  ), x=circle_size, just="left")
  circos.clear()
})

p1_grob <- as.ggplot(p1_grob)
print(p1_grob)
```
