...
extract_sites <- function(results, filtered_cpg_list, gender, cor_threshold, abs_threshold_young, abs_threshold_old, sd_threshold, extra_filter=TRUE) {
  ...
  significant_counts <- data.frame(age_window = character(), count = integer(), Gender = String(), sites = I(list()), stringsAsFactors=FALSE)
  ...
  for (i in seq(1, ncol(results), by=3)) {
    ...
    first_filter <- apply(results[, c(i, i+1, i+2)], 1, function(row) {
      ...
      !is.na(corr_res) && !is.na(sd_res) && abs(corr_res) >= cor_threshold && sd_reses < sd_threshold
    })
    
    first_sites <- rownames(results)[first_filter]
    
    second_sites <- results[temp[i+1] >= abs_threshold, ]
    
    if (extra_filter) {
      if (dim(second_sites)[0] < 100) {
